...
;(function(root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        root.WalletApi.FDUserAnchorTransactionRequest = factory(root.WalletApi.ApiClient)
    }
     ...
     * User Fee Delegation Anchoring Transaction Request Scheme
     * @alias FDUserAnchorTransactionRequest
     * @class
     * @param from {String} Klaytn account address sending a transaction
     * @param input {String} Data attached to a transaction (anchored to the main chain)
     * @param feePayer {String} Account address for fee delegation of transaction fee
     */
    const FDUserAnchorTransactionRequest = function(from, input, feePayer) {
        this.from = from
        this.input = input
     ...
     */
    FDUserAnchorTransactionRequest.constructFromObject = function(data, obj) {
        if (data) {
    ...
    /**
     * Data attached to a transaction (anchored to the main chain)
     * @type {String}
     * @memberof FDUserAnchorTransactionRequest
     */
    ...
    return FDUserAnchorTransactionRequest
})
