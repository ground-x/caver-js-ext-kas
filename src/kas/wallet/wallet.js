...
class Wallet {
    ...
    async migrateAccounts(chainId, accounts) {
        ...
        // Define a new keyring container for account migration.
        ...
        const keys = await this.apiInstances.key.key.keyCreation(chainId, { size: accounts.length })
        ...
        for (let i = 0; i < accounts; i++) {
            ...
            const factory = this.keyringFactory(el)
            ...
            await keyringContainer.sign(el.address, updateTx)
            ...
            // Remove the address from caver keyring container for memory optimization.
            keyringContainer.remove(el.address)
        }
