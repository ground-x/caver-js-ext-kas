...
describe('KIP17 API service', () => {
    ...
    //     const accounts = await caver.kas.wallet.getAccountList({ size: 3 })

    //     if (accounts.items.length < 3) {
    //         const generated = await caver.wallet.generate(3 - accounts.items.length)
    //         for (const acct of generated) {
    //             accounts.items.push({ address: acct.address })
    //         }
    //     }

    //     const toAddress = accounts.items[0].address
    //     const id = 0
    ...
    //     owner = toAddress
    //     tokenId = id
    //     sender = accounts.items[1].address
    //     to = accounts.items[2].address
